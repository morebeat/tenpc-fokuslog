document.addEventListener('DOMContentLoaded', async () => {
    ...
    function displayGamificationStats(user) {
        ...
        const currentBadge = badges.length > 0 ? badges[badges.length - 1] : null;
        const currentBadgeHtml = currentBadge
            ? `
                <div style="margin: 15px auto 5px; max-width: 360px; background: rgba(255,255,255,0.15); border-radius: 14px; padding: 12px 16px; display: flex; align-items: center; gap: 12px;">
                    <div style="font-size: 2.2em;">${badgeIcons[currentBadge.icon_class] || '🏅'}</div>
                    <div style="text-align: left;">
                        <div style="font-size: 0.8em; text-transform: uppercase; letter-spacing: 0.05em; opacity: 0.8;">Aktuelles Abzeichen</div>
                        <div style="font-weight: 700;">${currentBadge.name}</div>
                        <div style="font-size: 0.85em; opacity: 0.9;">${currentBadge.description}</div>
                    </div>
                </div>`
            : '<p style="margin: 15px 0 5px; font-size: 0.9em; opacity: 0.8;">Noch kein Abzeichen – jeden Tag zählt!</p>';
